...
def knuth_guess_fast(psw_pool, hints_pool):
    if len(psw_pool) == 1:
        ...
        for possible_next_guess in psw_pool_complete:
            ...
            if (min_cut_found > max_cut_found) or (
                    (min_cut_found == max_cut_found) and possible_next_guess in psw_pool):
...
def hints_calculator(psw_goal, psw_guess):
    black_pegs = len(
        [goal_element for goal_element, guess_element in zip(psw_goal, psw_guess) if goal_element == guess_element])
